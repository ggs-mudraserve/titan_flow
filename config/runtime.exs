...
config :titan_flow, TitanFlow.Repo,
  ...
  # Required for Supavisor transaction mode - don't use named prepared statements
  prepare: :unnamed,
  # Increased to 30 to support 20 workers + Web
  pool_size: String.to_integer(System.get_env("POOL_SIZE") || "30"),
  # Allow requests to queue for up to 5 seconds
  queue_target: 5000,
...
config :titan_flow,
       ...
       System.get_env("WEBHOOK_VERIFY_TOKEN", "titanflow_webhook_secret")
...
if config_env() == :prod do
  ...
  config :titan_flow, TitanFlowWeb.Endpoint,
    ...
    http: [
      # Bind to localhost; nginx handles public TLS traffic.
      # See https://hexdocs.pm/bandit/Bandit.html#t:options/0 for details.
      ip: {127, 0, 0, 1}
    ],

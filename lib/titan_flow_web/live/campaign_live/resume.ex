defmodule TitanFlowWeb.CampaignLive.Resume do
      ...
      campaign.senders_config && campaign.senders_config != [] ->
        Enum.map(campaign.senders_config, fn config ->
          %{
            ...
            template_ids: MapSet.new(config["template_ids"] || [])
          }
        end)
      ...
      campaign.phone_ids && campaign.phone_ids != [] ->
        ...
        Enum.map(campaign.phone_ids, fn phone_id ->
          %{
            id: generate_id(),
            phone_id: phone_id,
            template_ids: MapSet.new(campaign.template_ids || [])
          }
        end)
      
      # No existing config - start with empty sender
      true ->
        [%{id: generate_id(), phone_id: nil, template_ids: MapSet.new()}]
    end
  end
  ...
  def handle_event("validate", params, socket) do
    ...
    {:noreply, assign(socket, campaign_name: campaign_name, dedup_window_days: dedup_days, error: nil)}
  end
  ...
  defp do_start_campaign(socket, name, senders_config) do
    ...
    {:ok, updated_campaign} = Campaigns.update_campaign(campaign, %{
      name: name,

defmodule TitanFlow.Campaigns.AutoScaler do
  ...
  @db_throttle_threshold_ms 1_000  # Throttle if checkout > 1s
  
  # MPS tiers based on queue depth
  @queue_tiers [
    {15_000, 200},  # Very high queue -> aggressive
    {10_000, 150},  # High queue -> fast
    {5_000, 100},   # Medium queue -> normal
    {0, 80}         # Low queue -> conservative
  ]
  
  @emergency_mps 50  # When DB is overloaded
  ...
  def init(_opts) do
    ...
    {:ok, %{
      last_check: nil,
      active_phones: %{},        # phone_id => {current_mps, queue_depth}
  ...
  defp perform_scaling_check(state) do
    ...
    if map_size(active_phones) > 0 do
      Logger.debug("[AutoScaler] Active phones: #{map_size(active_phones)}, DB throttle: #{db_overloaded}")
    end
  ...
  defp apply_scaling_decisions(queues, db_overloaded) do
    Enum.map(queues, fn {phone_id, %{depth: depth}} ->
      target_mps = if db_overloaded do
        @emergency_mps
      else

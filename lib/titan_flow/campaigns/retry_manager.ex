defmodule TitanFlow.Campaigns.RetryManager do
  ...
  def process_exhausted_phone(phone_number_id, campaign_id) do
    Logger.info("RetryManager: Processing exhausted phone #{phone_number_id} for campaign #{campaign_id}")
    ...
    case find_active_phone(campaign_id) do
      {:ok, active_phone_id} ->
        ...
        requeued_count = Enum.reduce(failed_messages, 0, fn msg, count ->
        ...
        Logger.info("RetryManager: Re-queued #{requeued_count} messages to phone #{active_phone_id}")
        {:ok, requeued_count}
  ...
  defp find_active_phone(campaign_id) do
    ...
    # Get exhausted phones from Redis
    ...
    active_phone = Enum.find(phone_ids, fn id ->
      not MapSet.member?(exhausted_set, to_string(id))
    end)
    
    case active_phone do
      nil -> {:error, :no_active_phone}
      phone_id -> 
  ...
  def retry_template_failure(template_name, campaign_id, _new_template_id) do
    Logger.info("RetryManager: Retrying template failures for #{template_name} in campaign #{campaign_id}")
    ...
    case find_active_phone(campaign_id) do
      {:ok, active_phone_id} ->
        ...
        # 3. Re-queue them
        count = Enum.reduce(failed_messages, 0, fn msg, acc ->
      ...
      error ->
        Logger.error("RetryManager: No active phone to retry template messages: #{inspect(error)}")
        error
    end
  end
    ...
    case Redix.command(:redix, ["GET", retry_key]) do
      ...
      {:ok, count_str} ->
        ...
        if count < @max_retry_count do
          ...
          Logger.warning("RetryManager: Message #{message_log.meta_message_id} exceeded max retries (#{@max_retry_count})")
          :skip
        end
  ...
  defp do_requeue(message_log, active_phone_id, campaign_id, retry_key, retry_count) do
    # Build payload from message log
    # Note: We need the original payload, but we only have recipient_phone
    # This is a simplified version - full implementation would store original payload
    
    payload = Jason.encode!(%{
      "phone" => message_log.recipient_phone,
      "contact_id" => message_log.contact_id,
      "variables" => %{},  # Lost in current design - consider storing

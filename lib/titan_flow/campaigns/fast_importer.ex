defmodule TitanFlow.Campaigns.FastImporter do
  ...
  def import_csv(file_path, campaign_id) do
    ...
    try do
      ...
      csv_stream =
        case detect_format(file_path) do
  ...
  defp stream_zip(path) do
    ...
    case :zip.unzip(to_charlist(path), [:memory]) do
      {:ok, file_list} ->
        # Find first .csv file
        csv_file =
          Enum.find(file_list, fn {name, _data} ->
  ...
  defp import_via_copy(csv_stream, campaign_id) do
    ...
    try do
      Postgrex.transaction(conn, fn conn ->
        ...
        # DISTINCT ON (phone) ensures only first occurrence of each phone is inserted
        result =
          Postgrex.query!(
            conn,
            """

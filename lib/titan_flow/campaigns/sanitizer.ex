defmodule TitanFlow.Campaigns.Sanitizer do
  ...
  def apply_deduplication(campaign_id) do
    ...
    if campaign.dedup_window_days > 0 do
      Logger.info(
        "Sanitizer: Applying #{campaign.dedup_window_days}-day deduplication for campaign #{campaign_id}"
      )
      ...
      Logger.info(
        "Sanitizer: Removed #{skipped_count} contacts from campaign #{campaign_id} (contacted in last #{campaign.dedup_window_days} days) in #{elapsed}ms"
      )
